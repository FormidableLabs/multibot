...
// Permanent Comment: Hack to force `-h` output for README.md pasting to output
// `multibot` instead of `bin/multibot.js` (or anything else).
process.argv[1] = "multibot";
...
Validate.prototype = {
  ...
  authentication: function () {
    // Short-circuit if no-auth.
    if (this.argv.auth === false) {
      return this;
    }
  ...
  transform: function () {
    ...
    if (transform) {
      try {
        ...
        this.argv.transform = require(path.resolve(transform));
      } catch (err) {
  ...
  action: function () {
    ...
    if (action === "branch-to-pr") {
      if (this.argv.allowExisting) {
        // Allowing existing branches and PRs is too dangerous for
        // `branch-to-pr` because it cannot be simulated with `--dry-run` and
        // may do very bad things depending on the state of existing branches
        // or pull requests.
        //
        // It's safe for one-off actions because there is no cascading effect
        // of the root assumptions.
        //
        // In order to properly do this, we'd need to work backwards and
        // actually check if things exist _first_ and then if not allow a
        // simulated fallack. Too complicated for now, and dangerous so we
        // disallow for the time being.
        this._fail("Cannot enable `--allow-existing` for `branch-to-pr` action");
      }
    }
...
module.exports = {
  parse: function () {
    return yargs
      ...
      // Authentication: none
      .option("auth", {
        describe: "Use authentication (limited functionality without)",
        type: "boolean",
        default: true
      })
      ...
      .option("gh-host", {
        describe: "GitHub host URL (for enterprise)",
        type: "string"
      })
      .option("gh-path-prefix", {
        describe: "GitHub path prefix (for enterprise). E.g., '/api/v3'",
        type: "string"
      })

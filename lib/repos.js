    ...
    postDestBlobs: ["getSrcContents", function (results, cb) {
      ...
      async.map(srcBlobs, function (blob, blobCb) {
        ...
        if (blob.content.create || blob.content.update) {
          ...
          // Actual POST.
          // **Note**: Use `base64` encoding so we don't lose things like
          // trailing newlines which can happen with `utf-8`
          ...
          self._client.gitdata.createBlob({
            ...
            content: new Buffer(blob.content.new).toString("base64"),
            encoding: "base64" // Hard code base64
          }, function (err, res) {

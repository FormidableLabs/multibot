...
Repos.prototype.pullRequest = function (callback) {
  ...
  async.map(self._repos, function (repo, prCb) {
    ...
    // TODO: Handle "already created PR" `allowExisting`.
    ...
    self._repoObjs[repo].createPullRequest({
      ...
      console.log("TODO HERE createPullRequest", err, data);
      if (self._checkAndHandleError(err, "pullRequest.createPullRequest - " + repo, prCb)) {
        return;
      }
    ...
    prCb(null, {
      repo: repo,
      repoObj: self._repoObjs[repo].createPullRequest,

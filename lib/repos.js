...
Repos.prototype._read = function (branchSrc, callback) {
  ...
  async.map(lookups, function (obj, cb) {
    self._repoObjs[obj.repo].contents(branchSrc, obj.file, function (err, data) {
      data = data || {};
...
Repos.prototype._pullRequest = function (callback) {
  ...
  async.map(self._repos, function (repo, prCb) {
    ...
    // Abstract here for multiple calling scenarios.
    var handler = function (err, data) {
      data = data || {};
      var prExists = false;

      if (err) {
        console.log("TODO HERE ERR", JSON.stringify(err, null, 2));
      }

      if (err && err.error === EXISTING_PR && allowExisting) {
        prExists = true;
    ...
    //
    // Note: We're using a `catch` to handle the error
    var tmp = api.postPullRequest(repo, {
      title: self._title,
      body: self._msg,
      base: branchSrc,
      head: branchDest
    ...
    console.log("PROMISE KEYS", Object.keys(tmp));

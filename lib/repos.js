...
Repos.prototype.commit = function (callback) {
  var self = this;
  var branchSrc = self._branchDest; // Read from dest branch we're commiting to

  this._commit(branchSrc, function (err, results) {
    callback(err, err ? null : self._toDisplay(results));
  });
};
 ...
 * @returns {void}
 */
Repos.prototype._commit = function (branchSrc, callback) {
  var self = this;
  var branchDest = self._branchDest;
  var branchDestFull = "heads/" + branchDest;
  ...
  async.auto({
    ...
    getSrcContents: self._read.bind(self, branchSrc),

...
var FORBIDDEN = 403;
var NOT_FOUND = 404;
var EXISTING_PR = 422; // Unprocessable when existing PR.
...
var Repos = module.exports = function (opts) {
  ...
  this._repoParts = this._repos.map(function (repo) {
    var parts = repo.split("/");
    return { org: parts[0], repo: parts[1] }; // eslint-disable-line no-magic-numbers
  });
...
Repos.prototype._checkAndHandleError = function (err, msg, callback) {
  ...
  // TODO: Check if better way to detect rate limit.
  // TODO: Object is likely in wrong format now.
...
Repos.prototype._read = function (branchSrc, callback) {
  ...
  var lookups = self._repoParts.reduce(function (memo, parts) {
    return memo.concat(self._files.map(function (file) {
      return { org: parts.org, repo: parts.repo, file: file };
    }));
  }, []);
  ...
  async.auto({
    ...
    getSrcTrees: ["getSrcContents", function (cb, results) {
      ...
      async.map(self._repos, function (repo, repoCb) {
        ...
        repoObj.getRef(branchDestFull, function (refErr, ref) {
    ...
    updateBranch: ["postDestCommits", function (cb, results) {
      ...
      async.map(destCommits, function (destCommit, branchCb) {
        ...
        // Actual commit.
        api.postRef(repo, branchDestFull, destCommit.sha, function (err, data) {
...
Repos.prototype._pullRequest = function (callback) {
  ...
  async.map(self._repos, function (repo, prCb) {
    ...
    self._repoObjs[repo].createPullRequest({
      title: self._title,
      body: self._msg,
      base: branchSrc,
      head: branchDest
      ...
      // TODO: Need better fidelity with message here.
      if (err && err.code === EXISTING_PR && allowExisting) {
        prExists = true;
      } else if (self._checkAndHandleError(err, "pullRequest.createPullRequest - " + repo, prCb)) {
        return;
      }
...
Repos.prototype.branchToPr = function (callback) {
  ...
  async.auto({
    ...
    commit: ["branch", self._commit.bind(self, branchDest)],

    pullRequest: ["commit", self._pullRequest.bind(self)]
  }, function (err, results) {
    if (err) {
      callback(err);
      return;
